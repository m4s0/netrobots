...
class RobotStatus(object):
    ...
    def real_time_tick(self):
        ...
        The real time (human perceived) in seconds, the sistem waits before processing the next request. This differs from timeIncrement, because timeIncrement is the simulation time, and maps the accuracy of the simulation/robot reaction, while this maps how much the humans must wait for seeing the results. This value can vary, and it depends from the speed of network connection between robots. Usually it is a constant value.
